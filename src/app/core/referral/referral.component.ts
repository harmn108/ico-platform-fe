...
export class ReferralComponent implements OnInit, OnDestroy {
  ...
  ReferralEmailSubmit() {
    ...
    this.userService.referralEmailSubmit(this.emailForm.value.email, this.currentLanguage);
    this.referralEmailSubmitSub = this.userService.referralEmailSubmitChanged.subscribe(
      data => {
        this.answer = true;
        this.errorMessage = '';
