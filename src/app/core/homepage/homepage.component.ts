...
export class HomepageComponent implements OnInit {
    ...
    constructor(private router: Router,
        ...
        this.router.events.subscribe(event => {
            (event['url'] && ((event['url'].slice(4, 11) === 'profile') || (event['url'].slice(4, 10) === 'wallet'))) ? this.visaFooter = false : this.visaFooter = true;
            event['url'] === `/${this.languageService.language.value}` || event['url'] === '/' ? this.pageContent = true : this.pageContent = false;
        });
